...
bool
XFEM_geometric_cut_2d::isFirstPointInElem(const Elem* elem)
{
  Point p1(x0, y0, 0.0);
  unsigned int counter = 0;
  for (unsigned int i = 0; i < elem->n_nodes(); ++i)
  {
    unsigned int iplus1(i < elem->n_nodes()-1 ? i+1 : 0);
    Point node1 = (*elem->get_node(i));
    Point node2 = (*elem->get_node(iplus1));
    Point side = node2 - node1;
    Point normal(side(1), -side(0), 0.0);
    Point v = p1 - 0.5*(node1 + node2);
    if (v*normal <= 0.0 ) counter += 1;
  }
  if (counter == elem->n_nodes())
    return true;
  else
    return false;
}
...
Real
XFEM_geometric_cut_2d::distFirstPointToElem(const Elem* elem)
{
  Point p1(x0, y0, 0.0);
  Point center(0.0, 0.0, 0.0);
  for (unsigned int i = 0; i < elem->n_nodes(); ++i)
  {
    Point node = (*elem->get_node(i));
    center += node;
  }
  center *= (1.0/elem->n_nodes());
  return std::sqrt((p1-center).size_sq());
}

bool
XFEM_geometric_cut_2d::doesPassElem(const Elem* elem, Real time)
{
  //Use the algorithm described here to determine whether edges are cut by the cut line:
  //http://stackoverflow.com/questions/563198/how-do-you-detect-where-two-line-segments-intersect
