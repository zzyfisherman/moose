...
CrackFrontDefinition::isPointWithIndexOnIntersectingBoundary(const unsigned int point_index) const
{
  ...
  else
  {
    ...
    //the first and last point is assumed to be on the boundary
    unsigned int num_crack_front_points = getNumCrackFrontPoints();
...
CrackFrontDefinition::CrackFrontDefinition(const std::string & deprecated_name, InputParameters parameters) :
